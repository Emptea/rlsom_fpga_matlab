function analyze_model_vs_rtl(tp_num, model_sg, rtl_sg)
...
if numel(channels) > 1
    for ch = channels
        ...
        analyze_signals(model_sg_ch, rtl_sg_ch, name);
    end
else
    analyze_signals(model_sg, rtl_sg, tp_num.to_string());
end

end


function analyze_signals(model_sg, rtl_sg, name)
