function sg = fpga_txt2mat_for_ch(filename, ch_num, range_gate)

fprintf('Обработка файла: %s\n', filename);

[folder, name, ext] = fileparts(filename);
file = string(name) + string(ext);
out_filename = folder + "/" + name + "_ch" + ch_num + "_rg" + range_gate +".mat";
hdr_sz = 6;
n_sf = 40;
n_fft = 512;
...
if contains(file, "tp10")
    ...
    tp10_rank = fpga_fxp2double(sg) * n_sf * n_fft;
    save(out_filename, "tp10_rank"); 
elseif contains(file, "tp11")
    sg = fpga_read_res_file_for_ch(filename, [n_far_and_need + hdr_sz, 1], 1);
    tp11_apu = fpga_fxp2double(sg) * n_sf * n_fft;
...
elseif contains(file, "tp9")
    ...
    tp9_find = fpga_fxp2double(sg) * n_sf * n_fft;
